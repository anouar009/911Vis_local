...
(function (app, window, d3, undefined) {
  ...
  var pullData = function(callback) {
    d3.csv("911_clean.csv", function (err, data){
      if (err) return console.warn(err);

      callback(data.map(function (val) {
        val.date = new Date(val.date); //Create a real date object to use in the app
  ...
  var redrawChart = function () {
    ...
    for (var i = 0; i < types.length; i++) {
      typeCounts.push(
        displayedData.filter(function (item) { return item.description === types[i]; }).length
      );
    }
